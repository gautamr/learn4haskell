...
takeEven l = go 0 l
    where go _ [] = []
          go n (x:xs) = if even n then x: go (n + 1) xs else go (n + 1) xs
...
-}
contains n ls = filter (elem n) ls
...
♫ NOTE: The Haskell standard library already provides the "reverse"
  ...
  cheating!
-}
rewind l = go (drop (length l - 1) l) (take (length l - 1) l)
    where go accum [] = accum
          go accum xs = go (accum ++ drop (length xs - 1) xs) (take (length xs - 1) xs)
